...
if (
  ...
  $role = htmlspecialchars('professeur');
  ...
  if ($check == false) {
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
      ...
      header('Location:add_professeur.php?error=email');
    }
  } else {
    header('Location:add_professeur.php?error=already');
  }
}
...
<body>
  <div class="flex items-center min-h-screen p-6 bg-gray-50 dark:bg-gray-900">
    ...
    <form action="add_professeur.php" method="POST">
      ...
      <label class="block mt-4 text-sm">
        ...
        <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 
                  ...
                  form-input" name="email" type="email" placeholder="email@3il.fr" />
      </label>
      <label class="block mt-4 text-sm">
